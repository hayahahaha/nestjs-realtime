...
export class AuthenticationController {
  ...
  async LogIn(@Req() request: RequestWithUser, @Res() response: Response) {
    ...
    this.userService.setCurrentRefreshToken(refreshTokenCookie.token, user.id)
  ...
  async refresh(@Req() request: RequestWithUser, @Res() response: Response) {
    const accessTokenCookie = await this.authenticationService.getCookieWithJwtToken(request.user.id)
